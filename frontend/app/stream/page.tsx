...
export default function StreamPage() {
  ...
  const [participants] = useState([{ id: 1, name: "User 1", avatar: "👤" }]);
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      console.log("Tracks", stream.getTracks()); // Should NOT be empty
          ...
          <div className="lg:col-span-2">
            <Card className="bg-black/40 backdrop-blur-lg border-white/20 overflow-hidden">
              <div className="aspect-video relative bg-slate-800">
                ...
                {/* Camera Off Overlay (only if not loading & not camera on) */}
                {!isLoading && !isCameraOn && (
                  <div className="absolute inset-0 z-10 flex items-center justify-center text-slate-400 bg-black/70">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 gap-4">
              {participants.map((participant) => (
                <Card
                  key={participant.id}
                  className="bg-black/40 backdrop-blur-lg border-white/20"
                >
              ...
              {/* Empty slots */}
              {Array.from({ length: Math.max(0, 4 - 3) }).map((_, index) => (
                <Card
                  key={`empty-${index}`}
                  className="bg-black/20 backdrop-blur-lg border-white/10 border-dashed"
                >
